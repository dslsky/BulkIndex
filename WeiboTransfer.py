...
logging.basicConfig(filename='log/weibo_transfer.log',
                    format='%(asctime)s - %(name)s - %(levelname)s - %(module)s :%(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S %p', level=logging.WARNING)

class GetWeiboTransfer(object):
    ...
    def run(self):
        ...
        cunzai = 0
        while True:
            rowkey = self.redis_con.get_rowkey("wb_transfer")
            if rowkey == None:
            ...
            boo = self.es.exists("wb_transfer", "sino", rowkey)
            action = {
                "_index": "wb_transfer",
                "_type": "sino",
                "_id": "",
            ...
            if boo:
                map = self.hbase_con.getResultByRowkey("WEIBO_TRANSFER_TABLE", rowkey, "wb_transfer",param)
                if not map:
                    continue
                cunzai = cunzai + 1
                ...
                map = self.hbase_con.getResultByRowkey("WEIBO_TRANSFER_TABLE", rowkey, "wb_transfer")
                if not map:
                    continue
                action['_source'] = map
            action['_id'] = rowkey
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:wb_transfer,\terror:" + str(e)
            logging.error(log_info)
            helpers.bulk(self.es, action_list)
        logging.warning("提交成功：%d条数据" % len(action_list))

if __name__=="__main__":
    getWeiboTransfer = GetWeiboTransfer()
    getWeiboTransfer.run()