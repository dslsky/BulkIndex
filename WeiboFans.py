...
logging.basicConfig(filename='log/weibo_fans.log',
...
class GetWeiboFans(object):
    ...
    def run(self):
        ...
        start = int(time.time())
        while True:
            rowkey = self.redis_con.get_rowkey("wb_fans")
            if rowkey == None:
            ...
            map = self.hbase_con.getResultByRowkey("WEIBO_FANS_TABLE", rowkey, "wb_fans")
            if not map:
                continue
            action = {
                "_index": "wb_fans",
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:wb_fans,\terror:" + str(e)
