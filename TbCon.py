...
logging.basicConfig(filename='log/tb_con.log',
...
class GetTbCon(object):
    ...
    def run(self):
        ...
        while True:
            ...
            if boo:
                cunzai = cunzai + 1
                map = self.hbase_con.getResultByRowkey("TB_CON_TABLE", rowkey, "tb_con",param)
                if not map:
                    continue
                action_list.append({
                    "_op_type":"update",
                ...
                map = self.hbase_con.getResultByRowkey("TB_CON_TABLE", rowkey, "tb_con")
                if not map:
                    continue
                action_list.append({
                    "_index": "tb_con",
