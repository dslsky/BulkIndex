...
logging.basicConfig(filename='log/wechat_group.log',
                    format='%(asctime)s - %(name)s - %(levelname)s - %(module)s :%(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S %p', level=logging.INFO)

class GetWechatGroup(object):
    ...
    def run(self):
        ...
        start = int(time.time())
        while True:
            rowkey = self.redis_con.get_rowkey("wx_group")
            if rowkey == None:
            ...
            map = self.hbase_con.getResultByRowkey("WECHAT_GROUP_TABLE", rowkey, "wx_group")
            if not map:
                continue
            action = {
                "_index": "wx_group",
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:wx_group,\terror:" + str(e)
