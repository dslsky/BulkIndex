...
logging.basicConfig(filename='log/audio.log',
...
class Audio(object):
    ...
    def run(self):
        ...
        start = int(time.time())
        while True:
            rowkey = self.redis_con.get_rowkey("audio")
            if rowkey == None:
            ...
            map = self.hbase_con.getSuanfaResultByRowkey("AUDIO_DATA_TABLE", rowkey, "audio")
            if not map:
                continue
            action = {
                "_index": "audio",
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:audio,\terror:" + str(e)
