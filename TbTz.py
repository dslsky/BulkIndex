...
logging.basicConfig(filename='log/tb_tz.log',
                    format='%(asctime)s - %(name)s - %(levelname)s - %(module)s :%(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S %p', level=logging.WARNING)

class GetTbTz(object):
    ...
    def run(self):
        ...
        cunzai = 0
        while True:
            rowkey = self.redis_con.get_rowkey("tb_tz")
            if rowkey == None:
            ...
            boo = self.es.exists("tb_tz","sino",rowkey)
            if boo:
                cunzai = cunzai + 1
                map = self.hbase_con.getResultByRowkey("TB_TZ_TABLE", rowkey, "tb_tz",param)
                if not map:
                    continue
                action_list.append({
                    "_op_type":"update",
                ...
                map = self.hbase_con.getResultByRowkey("TB_TZ_TABLE", rowkey, "tb_tz")
                if not map:
                    continue
                action_list.append({
                    "_index": "tb_tz",
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:tb_tz,\terror:" + str(e)
