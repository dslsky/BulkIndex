...
logging.basicConfig(filename='log/tb_origin.log',
                    format='%(asctime)s - %(name)s - %(levelname)s - %(module)s :%(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S %p', level=logging.WARNING)

class GetTbOrigin(object):
    ...
    def run(self):
        ...
        cunzai = 0
        while True:
            rowkey = self.redis_con.get_rowkey("tb_origin")
            if rowkey == None:
            ...
            boo = self.es.exists("tb_origin","sino",rowkey)
            if boo:
                cunzai = cunzai + 1
                map = self.hbase_con.getResultByRowkey("TB_ORIGIN_TABLE", rowkey, "tb_origin",param)
                if not map:
                    continue
                action_list.append({
                    "_op_type":"update",
                ...
                map = self.hbase_con.getResultByRowkey("TB_ORIGIN_TABLE", rowkey, "tb_origin")
                if not map:
                    continue
                action_list.append({
                    "_index": "tb_origin",
