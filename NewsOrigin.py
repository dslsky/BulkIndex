...
logging.basicConfig(filename='log/xw_origin.log',
                    format='%(asctime)s - %(name)s - %(levelname)s - %(module)s :%(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S %p', level=logging.WARNING)

class GetNewsOrigin(object):
    ...
    def run(self):
        ...
        cunzai = 0
        while True:
            rowkey = self.redis_con.get_rowkey("xw_origin")
            if rowkey == None:
            ...
            boo = self.es.exists("xw_origin", "sino", rowkey)
            action = {
                "_index": "xw_origin",
                "_type": "sino",
                "_id": "",
            ...
            if boo:
                map = self.hbase_con.getResultByRowkey("NEWS_ORGIN_TABLE", rowkey, "xw_origin",param)
                if not map:
                    continue
                ...
                map = self.hbase_con.getResultByRowkey("NEWS_ORGIN_TABLE", rowkey, "xw_origin")
                if not map:
                    continue
                self.es.index(index="xw_origin",doc_type="sino",id=rowkey,body=map)
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:xw_origin index,\terror:" + str(e)
