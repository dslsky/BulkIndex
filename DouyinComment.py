...
logging.basicConfig(filename='log/douyin_comment.log',
                    format='%(asctime)s - %(name)s - %(levelname)s - %(module)s :%(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S %p', level=logging.WARNING)

class GetDouyinComment(object):
    ...
    def run(self):
        ...
        start = int(time.time())
        while True:
            rowkey = self.redis_con.get_rowkey("dy_comment")
            if rowkey == None:
            ...
            boo = self.es.exists("dy_comment", "sino", rowkey)
            action = {
                "_index": "dy_comment",
                "_type": "sino",
                "_id": "",
            ...
            if boo:
                map = self.hbase_con.getResultByRowkey("DOUYIN_COMMENT_TABLE", rowkey, "dy_comment",param)
                if not map:
                    continue
                ...
                map = self.hbase_con.getResultByRowkey("DOUYIN_COMMENT_TABLE", rowkey, "dy_comment")
                if not map:
                    continue
                try:
                    self.es.index(index="dy_comment", doc_type="sino", id=rowkey, body=map)
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:dy_comment,\terror:" + str(e)
            logging.error(log_info)
            helpers.bulk(self.es, action_list)
...
if __name__=="__main__":
    getDouyinComment = GetDouyinComment()
    getDouyinComment.run()