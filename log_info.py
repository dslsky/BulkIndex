...
class LogInsertInfo(object):
    ...
    def run(self):
        ...
        cunzai = 0
        while True:
            rowkey = self.redis_con.get_yy_nb_rowkey("es:log:insert:info")
            if rowkey == None:
            ...
            if len(rowkey) > 500:
                ...
                continue
            boo = self.es.exists("log_info","sino",rowkey)
            if boo:
                cunzai = cunzai + 1
                map = self.hbase_con.getResultByRowkey("log_info", rowkey)
                if not map:
                    continue
                action_list.append({
                    "_op_type":"update",
