...
logging.basicConfig(filename='log/xw_comment.log',
...
class GetNewsComment(object):
    ...
    def run(self):
        ...
        cunzai = 0
        while True:
            rowkey = self.redis_con.get_rowkey("xw_comment")
            if rowkey == None:
            ...
            if len(rowkey) > 500:
                ...
                continue
            boo = self.es.exists("xw_comment", "sino", rowkey)
            action = {
                "_index": "xw_comment",
                "_type": "sino",
                "_id": "",
            ...
            if boo:
                map = self.hbase_con.getResultByRowkey("NEWS_COMMENT_TABLE", rowkey, "xw_comment",param)
                if not map:
                    continue
                ...
                map = self.hbase_con.getResultByRowkey("NEWS_COMMENT_TABLE", rowkey, "xw_comment")
                if not map:
                    continue
                action['_source'] = map
            action['_id'] = rowkey
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:xw_comment,\terror:" + str(e)
