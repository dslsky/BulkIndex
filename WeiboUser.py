...
logging.basicConfig(filename='log/weibo_user.log',
                    format='%(asctime)s - %(name)s - %(levelname)s - %(module)s :%(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S %p', level=logging.WARNING)

class GetWeiboUser(object):
    ...
    def run(self):
        ...
        cunzai = 0
        while True:
            rowkey = self.redis_con.get_rowkey("wb_user")
            if rowkey == None:
            ...
            boo = self.es.exists("wb_user", "sino", rowkey)
            action = {
                "_index": "wb_user",
                "_type": "sino",
                "_id": "",
            ...
            if boo:
                map = self.hbase_con.getResultByRowkey("WEIBO_USER_TABLE", rowkey, "wb_user",param)
                if not map:
                    continue
                cunzai = cunzai + 1
                ...
                map = self.hbase_con.getResultByRowkey("WEIBO_USER_TABLE", rowkey, "wb_user")
                if not map:
                    continue
                action['_source'] = map
            action['_id'] = rowkey
    ...
    def commit(self,action_list):
        try:
            ...
            log_info = "index:wb_user,\terror:" + str(e)
